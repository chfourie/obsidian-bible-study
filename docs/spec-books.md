...
- **Manifest**: the existing `ModuleManifest` extended with `kind: 'book'` plus a required-when-book `book` sub-object — `{ number, editionCode, author, year, abbreviation, aliases?, sections: [{chapter, name, paragraphs, named?}] }`. One type, one store, one discovery path; `isTranslationManifest()` keeps translation-only code paths books-free. Books are published at `BOOK_MODULE_FORMAT_VERSION`, which runs ahead of the shared `MODULE_FORMAT_VERSION` on the same field so a format gain of theirs alone (v6: Headings, and the Part each section sits under) leaves installed translations untouched; capability flags all false/absent; `source`/`sourceChecksum` filled at download exactly like BSB.
