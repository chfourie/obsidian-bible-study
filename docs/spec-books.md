...
- **Linkify policy**: explicit citations only — the parser handles the uniform parenthetical patterns (`(John v. 19)`: roman chapter, arabic verse; lenient on the known missing-period case). A build-time **overrides file** supports *fix* (supply ranges for a mangled/missed ref) and *suppress* (false positive), keyed by chapter.paragraph + matched text; an override that no longer matches fails the build. Unreferenced allusions are never hand-annotated — Murray didn't cite them, we don't.
