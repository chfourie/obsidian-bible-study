...
- **Strong's dictionaries** (number→derivation/definition): **license verified 2026-08-20** (ticket #82). `openscriptures/strongs` has no LICENSE file and no `license` field in `package.json`; terms appear only in file headers, and they disagree: `greek/strongs-greek-dictionary.js` and `hebrew/strongs-hebrew-dictionary.js` both claim **CC-BY-SA**, while the OSIS header of `hebrew/StrongHebrewG.xml` states **Public Domain**. [openscriptures/HebrewLexicon](https://github.com/openscriptures/HebrewLexicon) states its terms in `readme.md`: files released under **CC BY 4.0**, with the underlying BDB and Strong's Hebrew text noted as public domain, asking credit for the Open Scriptures Hebrew Bible Project. The 1890/1894 text itself is public domain by age; the CC claims attach only to the modern transcription layer. **Decision: usable.** Hebrew derivations are taken from HebrewLexicon's `HebrewStrong.xml` (CC BY 4.0); Greek from `greek/StrongsGreekDictionaryXML_1.4/strongsgreek.xml`, deliberately **not** the JS build, so the CC-BY-SA claim over the same public-domain text is never inherited. No non-commercial clause anywhere. STEPBible's TBESH/TBESG (CC BY 4.0) remain the brief-lexicon layer.
...
- kaiserlik/kjv in-repo license file. (openscriptures/strongs and HebrewLexicon: resolved above.)
