...
- **Study Panel row:** members that fall outside the passage in view, then the summary, then two icons — edit (opens the strip seeded with all members and the summary) and note (opens the note; modifier click → new pane). The note icon is filled when the body has non-whitespace content and outlined otherwise, tooltips "Open note (has notes)" / "Open note". Template boilerplate counts as content. Row order per the existing Bible-order-led-by-the-viewed-chapter rule. Refresh flows from vault index events.
