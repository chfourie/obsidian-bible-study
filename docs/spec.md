...
- **Option tokens:** translation id, display keyword (`inline` | `block`) and the anchor keyword (`anchor`) in any order after the reference — disjoint closed vocabularies, each matched case-insensitively, first of a kind wins. Bare (no keyword) = chip only. A whole-book reference takes option tokens once at least one of them is one, the rest highlighted as invalid like any reference's; when no token classifies, the braces are plain text, keeping `{Mark Twain}` out of the grammar.
- **Anchor rule:** `anchor` makes a full reference a Pinned Anchor: every later relative reference (`{:3}`) in the note body resolves against the nearest Pinned Anchor before it, even when an unpinned full reference stands closer; with no Pinned Anchor earlier, the nearest full reference anchors. A later pin replaces the earlier and holds to the end of the body. Any full reference can be pinned, including whole-book, whole-chapter and Book citations; on a relative reference `anchor` is an invalid token. An invalid pinned reference and a frontmatter one never anchor. The pinned chip looks no different.
...
A note author's own quotation of Christ, written `c"Abide in me"` or `c“Abide in me”`. Grammar: a lowercase `c` at a word start (line start, or after whitespace or punctuation), immediately followed by a straight `"` or curly `“` opening mark; the quote ends at the first closing mark of the same kind (`"` or `”`) within the same paragraph. A paragraph ends at a blank line, a fence, a heading, a list item, a table row or a deeper blockquote line, and a table cell is a paragraph of its own; a plain continuation line, whether indented or under a `>`, stays in the paragraph it follows. Anything else — mid-word `c` (`Isaac"…"`), capital `C"`, a closing mark of the other kind, an unterminated quote, or one closing only in a later paragraph — is plain text. Single quotes never take part.

Rendering: the `c` is hidden and the quote, marks included, is wrapped in the red-letter class of §3.4, so it uses the same CSS variable as translation red-letter; no setting, reader option or new CSS. Inline markdown inside the quote renders as usual and stays red; a reference chip inside it renders as its usual chip. An ellipsis inside the quote — a run of three or more dots or the one-character `…` — is the author's elision, not a word of Christ, and keeps the normal text colour on every rendering path. Several Christ Quotes in one paragraph each render independently. Same in Reading mode, Live Preview (the `c` reappears only while the cursor or selection touches the quote; the colour stays) and the Study Panel's rendered Annotation bodies; Source mode shows the raw text.

Exclusions: inline code, fenced code blocks and frontmatter are never scanned, as for references. Escape: a backslash before the opening mark (`c\"…"`) keeps the text literal, `c"…"` with no backslash and no colour, detected from note source as reference escapes are. Before a curly mark (`c\“…”`) markdown leaves the backslash in the rendered text, since it only swallows `\` before ASCII punctuation; the render strips it all the same, so both escapes read alike. Christ Quotes are decoration only — never an Occurrence, never indexed, no effect on Mentions, Annotations or Cross-References. The reader's Red letter option and the Derived red letter setting govern translation text only and never hide a Christ Quote.
...
A **Page Break** (CONTEXT.md) is a body line whose trimmed text is exactly `===`, with a blank line or the note edge before it; frontmatter counts as the edge, so the body's first line can be one. Text may follow on the very next line and starts the new page; a marker that another `===` or `---` line underlines is a heading, not a Page Break. Lines in frontmatter, fenced code, list items, tables, blockquotes and callouts are never Page Breaks; spaces around the marker are tolerated, but a marker indented four or more spaces, or by a tab, is an indented code block and never a Page Break; `=== more` is a paragraph; `===` under a line of text stays the setext heading underline it always was; `===` right under a heading, a horizontal rule or a fence close renders as a paragraph of plain text and is no Page Break; `\===` is the escape and renders as the plain text `===`; `---` stays a horizontal rule.

Reading view: the post-processor replaces a top-level paragraph holding only the marker (or lifts the marker line off the front of the paragraph it opens) with one block element carrying a plugin class, the Lucide `separator-horizontal` icon and the label "page break". On screen it shows as a dashed rule in the theme's rule colour with a centered pill in the reference-chip family, muted text, not selectable. The element carries a trailing modifier when no non-blank line follows it in the note source. Obsidian's PDF export renders through reading view, so a print media rule is the whole export form: the element's visuals vanish (no height, rule or pill) and `break-after: page` turns the page; the trailing form breaks nothing, so two consecutive Page Breaks yield a blank page and one at the end of the note never adds an empty trailing page.
...
- **Reader display:** one folded row per annotation in the Study Panel, titled by its scripture address with the note's own name muted beside it — the slot References give the translation label — so two annotations on one address are told apart while folded, with the References row grammar — chevron and heading toggle it by click or keyboard, and an open row renders the note's whole markdown body in the panel's own scroll (no inner scroll box), read-only. Which rows stand open is remembered per tab for the tab's lifetime, keyed by the note's path, so a re-sort or a save changes no fold; a tab the panel has no memory of starts its annotations folded, or open where the global “Annotations start expanded” setting (default off) says so — flipping that setting reaches only the tabs that have arranged no annotation row, leaving the rest as the reader left them, and References rows start folded whatever it says. The Annotations heading carries the collapse-all / expand-all pair the References heading carries, acting on the annotation rows alone, beside the reader's add-annotation action. The edit icon stands in the row's trailing slot, where References place the open-in-reader icon, shown while the row's head is hovered or the icon focused, and opens the note in an editor split — a new pane with the Cmd/Ctrl modifier; modify events refresh on save. No rule divides the rows: they keep the References rows' spacing. Ordering is a global setting: creation date oldest-first (default) or file path A→Z.
- **In-note intersection surface:** annotations appear there too, grouped first ("Annotations", then "Mentions"). The count indicator and its panel are screen-only: a print media rule hides them, so a PDF export shows the rendered reference without the ● / ◆ counts.

## 5a. Cross-references

- **Concept:** a symmetric, n-ary connection between References (CONTEXT: Cross-Reference); a vault note per connection (ADR 0015 supersedes 0003).
- **Schema:** three frontmatter keys. `type: cross-reference` declares the note; `refs` is a YAML list of grammar strings without braces, one per member; `summary` is a one-line string. Canonicalized when the plugin writes them (grammar form, never the citation label); any valid grammar string accepted hand-typed. Missing `type` or missing `refs` → not a cross-reference, silently (body still indexes as a mention). A note carrying `ref` as well is also an Annotation; both index. A member that fails to parse (typo, uninstalled Book) is skipped but preserved verbatim through every plugin rewrite, keeping its place in the list — its original index, or trailing the list once the list is shorter than that — and, where there are several, their relative order. Fewer than two parseable members still indexes; the two-member floor is a strip rule only.
- **Indexing:** each parseable member is an occurrence with source `cross-reference-frontmatter`, position 0. A cross-reference note is never a Mention, whatever its body references, and marks no verse and counts in no in-note intersection — unless it carries `ref` too, when it marks and counts as the Annotation it also is. A declared note stays indexed with no parseable member at all, so a later edit reaches it.
- **Folder & filenames:** one configurable folder (default `Cross-References/`), created on demand, organisation-only. Filename = first two members' labels joined with ` + `, then ` (+n)` for further members, `:` → `.`; collisions suffix ` 1`, ` 2`, …. Regenerated after a member edit only while the current name is the one the plugin last generated for the members the note held — members as written, so a member the plugin can no longer parse still counts toward the `(+n)`, and a trailing number reads as the plugin's own collision suffix, so a suffixed note follows its members without taking a further suffix; a hand-renamed note, and one whose first two members include text the plugin cannot parse, keep their name. A summary-only edit never renames. The rename goes through Obsidian's file manager, so links to the note follow it.
- **Template:** optional template-file setting (default none). Create copies the template then sets the three keys, overwriting any of them in the template. A configured path with no file there is no error: the note is composed without a template.
- **Body:** free-form, the user's own. The plugin never writes it after create. Rewrites touch only the `refs` block and `summary` key; every other key, key order and the body stay byte-identical.
- **Create/edit/delete:** the reader's collection strip is the sole create surface (no command). It gathers members and the summary on create and on edit. Delete moves the note to trash via Obsidian's configured trash behaviour. A save or delete the vault refuses leaves the strip open, showing "Could not save: …" / "Could not delete: …".
- **Study Panel row:** every member — the passage in view's own leading, so what is on screen anchors the row — then the summary, then two icons — edit (opens the strip seeded with the note's parseable members and its summary) and note (opens the note beside the reader; modifier click → new tab, as a member reference opens). The note icon is filled when the body has non-whitespace content and outlined otherwise, tooltips "Open note (has notes)" / "Open note". Template boilerplate counts as content. Row order per the existing Bible-order-led-by-the-viewed-chapter rule. Refresh flows from vault index events.
- **In-note surface:** a cross-reference note focused as a tab lists its members as the passages it references, via the frontmatter-occurrence path annotations already use.
...
5. **Annotations** — Folder (folder-suggest, default `Annotations/`), Template file (file-suggest, default none), Display ordering (creation oldest-first default / path A→Z), Annotations start expanded (toggle, default off).
6. **Cross-references** — Folder (folder-suggest, default `Cross-References/`), Template file (file-suggest, default none).
7. **Mentions** — Excluded folders: a folder-suggest entry adds a vault folder to a list, each listed folder with a Remove. A note in an excluded folder, or in any folder below it, is never a Mention on any surface — the Study Panel's Mentions section, the reader's ◆ markers and counts, the in-note intersection panel — while what it declares (Annotation, Cross-Reference) still shows. Folders match on whole path segments (`Journal` excludes `Journal/2026/…`, not `Journal Notes/…`); entries are stored trimmed of blanks and slashes, once each, blanks dropped. The index keeps the notes, filtering at query time, so a note moved out or a folder struck from the list reappears at once, and open surfaces refresh without a restart.
...
7. **Annotations** — schema, create flows, reader/in-note display (§5); cross-references as sibling notes (§5a).
