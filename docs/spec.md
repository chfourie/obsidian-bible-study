...
- **Folder & filenames:** one configurable folder (default `Cross-References/`), created on demand, organisation-only. Filename = first two members' labels joined with ` + `, then ` (+n)` for further members, `:` → `.`; collisions suffix ` 1`, ` 2`, …. Regenerated after a member edit only while the current name equals the name last generated by the plugin; a hand-renamed note keeps its name.
- **Template:** optional template-file setting (default none). Create copies the template then sets the three keys, overwriting any of them in the template.
...
- **Create/edit/delete:** the reader's collection strip is the sole create surface (no command). It gathers members and the summary on create and on edit. Delete moves the note to trash via Obsidian's configured trash behaviour.
- **Study Panel row:** every member — the passage in view's own leading, so what is on screen anchors the row — then the summary, then two icons — edit (opens the strip seeded with all members and the summary) and note (opens the note; modifier click → new pane). The note icon is filled when the body has non-whitespace content and outlined otherwise, tooltips "Open note (has notes)" / "Open note". Template boilerplate counts as content. Row order per the existing Bible-order-led-by-the-viewed-chapter rule. Refresh flows from vault index events.
