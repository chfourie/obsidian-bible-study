...
- **Reader display:** one folded row per annotation in the Study Panel, titled by its scripture address, with the References row grammar — chevron and heading toggle it by click or keyboard, and an open row renders the note's whole markdown body in the panel's own scroll (no inner scroll box), read-only. Which rows stand open is remembered per tab for the tab's lifetime, keyed by the note's path, so a re-sort or a save changes no fold; a tab the panel has no memory of starts its annotations folded. The edit icon opens the note in an editor split; modify events refresh on save. Ordering is a global setting: creation date oldest-first (default) or file path A→Z.
