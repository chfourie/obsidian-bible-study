...
**Decision:** every cross-reference is a vault note. Frontmatter declares `type: cross-reference`, the member `refs` (grammar strings, canonicalized on plugin write, hand-typed accepted) and a one-line `summary`; the body is the user's own. Identity is the note path; the filename is generated for recognisability, bounded to the first two members plus a count, and read for one purpose only — deciding whether a member edit may regenerate it. Members index as occurrences in the existing vault index, so surfacing is the same intersection query annotations use and the store, its data file and the index exclusion all go away.
