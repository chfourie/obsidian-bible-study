...
### Anchor
The full Reference earlier in the same note body that a Relative Reference resolves against: the nearest Pinned Anchor if any stands earlier, otherwise the nearest full reference. Only full references anchor — a relative reference never anchors another, and an invalid one never breaks the chain. Frontmatter references never anchor.

### Pinned Anchor
A full Reference carrying the `anchor` option (`{John 15 anchor}`), declaring itself the passage under discussion. Every later Relative Reference in the note body resolves against the nearest Pinned Anchor before it, even when an unpinned full reference stands closer — so a note can cite other scripture without changing what `{:3}` refers to. A later Pinned Anchor replaces it; nothing unpins short of the end of the note body. Any reference that can anchor can be pinned, including whole-book and Book citations; on a Relative Reference the option is invalid, since it never anchors. Looks no different from an unpinned reference.
