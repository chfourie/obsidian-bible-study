...
### Word Cloud
The ten Strong's Families occurring most often in the chapter on screen, shown in the Study Panel as the family's most frequent Rendering in that chapter over its transliteration (gloss, lemma and count in the tooltip), each sized by its count and listed one per line, most frequent first. Counted from the viewed translation when it is a Tagged Translation, otherwise from an installed Tagged Translation named beside the cloud; absent for Books and while Strong's is not enabled. Everything counts except the Cloud Exclusions. Tapping a word opens its menu: toggle Occurrence Emphasis, open its Word Study Panel, or exclude it.
